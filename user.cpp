...
void user::checkReminders()
{
    ...
    {
        if (events[i].getReminded() == false && (Time_Date_Maneger::compTime(events[i].getReminderDate(), Time_Date_Maneger::getDateTime()) != 1)
            && (Time_Date_Maneger::compTime(events[i].getStartDate(), Time_Date_Maneger::getDateTime()) != -1))
        {
            cout << "\nREMINDER!\n";
