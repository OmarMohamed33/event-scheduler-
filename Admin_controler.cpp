...
#include "Manager_control.cpp"

using namespace std;
bool sortComparator(Event e1, Event e2) {
    //sort by start date
    if (Time_Date_Maneger::compTime(e1.getStartDate(), e2.getStartDate()) == 1)
        return 0;
    return 1;
}
bool sortbyReminderComparator(Event e1, Event e2) {
    if (Time_Date_Maneger::compTime(e1.getReminderDate(), e2.getReminderDate()) == 1)
        return 0;
    return 1;
}
bool canBeAdded(Event newEvent, vector<Event> events) {
    ...
    {
        if (Time_Date_Maneger::compTime(newEvent.getStartDate(), events[i].getEndDate()) == 1)
            return true;
        else if (Time_Date_Maneger::compTime(newEvent.getEndDate(), events[i].getStartDate()) == -1)
            return true;
    }
    return false;
}
Admin_controler::Admin_controler(vector<user>& users)
{
    this->users = users;
    landingPage();
}

void Admin_controler::Login()
{
    bool success = false;
    while (true) {
...
void Admin_controler::driver()
{
    Manager_control::writeUsers(this->users);
    Manager_control::readEvents(this->usr.events, usr.getName());
    bool exit = false;
    while (!exit)
    {
        switch (Admin_controler::mainMenu())
        {
        case 1:
            add_event();
            break;
...
void Admin_controler::landingPage()
{
    int input;
...
void Admin_controler::add_event()
{
    Event e;
    string name, place;
    ...
    while (true)
    {
        ...
        else
        {
            ...
            if (Time_Date_Maneger::compTime(e.getEndDate(), e.getStartDate()) != 1)
            {
    ...
    while (true)
    {
        ...
        else
        {
            ...
            if (Time_Date_Maneger::compTime(e.getReminderDate(), e.getStartDate()) != -1)
            {
...
bool Admin_controler::check_date(int day, int month, int year, int minutes, int hours)
{
    tm t = Time_Date_Maneger::getDateTime();
...
void Admin_controler::del_event() {
...
void Admin_controler::disp_done_event() {
...
void Admin_controler::disp_event() {
    if (this->usr.events.empty())
    {
...
void Admin_controler::update_event() {
...
Admin_controler::~Admin_controler() {}